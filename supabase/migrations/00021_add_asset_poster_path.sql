-- Add poster_path column to assets for video poster frames.
-- Populated at upload time by extracting the first visible frame client-side.
alter table public.assets
  add column poster_path text;

-- Rebuild the RPC to pass through poster_path.
  ...
  if p_assets is not null then
    for v_asset in select value from jsonb_array_elements(p_assets) loop
      insert into public.assets (
        ...
        thumb_hash, dominant_color, poster_path
      ) values (
        v_post_id,
        v_asset->>'file_path',
        v_asset->>'mime_type',
