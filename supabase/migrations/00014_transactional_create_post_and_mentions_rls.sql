-- Slice 3 (issue #42): atomic post creation + tighter mentions RLS.

-- 1. Atomic post creation
-- Wraps posts + tags + post_tags + assets in a single function so a failure
-- mid-sequence no longer leaves orphans. RLS applies via security invoker.
...
grant execute on function public.create_post_with_relations(text, text, text, text[], jsonb)
  to authenticated;

-- 2. Mentions RLS tightening
-- Previously: any authenticated user could insert any (comment_id, user_id)
-- row. Now: only the comment author can create/delete mention rows for their
-- own comments. (Uniqueness constraint was already defined in 00007.)
drop policy if exists "Authenticated can create mentions" on public.mentions;
